...
QDoubleSpinBox *optionalSpin(double max, const QString &suffix, int decimals = 1)
{
    ...
    spin->setSpecialValueText(QString::fromUtf8("不限"));
...
QString valueOrUnknown(double value, const QString &unit, int decimals = 1)
{
    return threeDHasValue(value)
        ...
        : QString::fromUtf8("未公开");
}

QString intOrUnknown(int value)
{
    return value >= 0 ? QString::number(value) : QString::fromUtf8("未公开");
}
...
QString fovRange(double nearValue, double referenceValue, double farValue, const QString &unit)
{
    ...
    if (threeDHasValue(nearValue))
        values.append(QString::fromUtf8("近端 %1%2").arg(nearValue, 0, 'f', 1).arg(unit));
    if (threeDHasValue(referenceValue))
        values.append(QString::fromUtf8("参考 %1%2").arg(referenceValue, 0, 'f', 1).arg(unit));
    if (threeDHasValue(farValue))
        values.append(QString::fromUtf8("远端 %1%2").arg(farValue, 0, 'f', 1).arg(unit));
    return values.isEmpty() ? QString::fromUtf8("未公开") : values.join(QString::fromUtf8("，"));
}

QString geometrySummary(const ThreeDCameraSpec &spec)
{
    return QString::fromUtf8("X：%1；Y：%2；Z：%3；CD：%4")
...
QString qualitySummary(const ThreeDCameraSpec &spec)
{
    ...
    if (threeDHasValue(spec.zRepeatabilityUm))
        parts.append(QString::fromUtf8("Z重复 %1 um").arg(spec.zRepeatabilityUm, 0, 'f', 2));
    if (threeDHasValue(spec.zResolutionUm))
        parts.append(QString::fromUtf8("Z分辨 %1 um").arg(spec.zResolutionUm, 0, 'f', 2));
    if (threeDHasValue(spec.measurementAccuracyUm))
        parts.append(QString::fromUtf8("测量精度 %1 um").arg(spec.measurementAccuracyUm, 0, 'f', 1));
    if (threeDHasValue(spec.zLinearityPercentOfRange))
        parts.append(QString::fromUtf8("Z线性 +/- %1%MR").arg(spec.zLinearityPercentOfRange, 0, 'f', 3));
    return parts.isEmpty() ? QString::fromUtf8("未公开") : parts.join(QString::fromUtf8("；"));
}
...
QString speedSummary(const ThreeDCameraSpec &spec)
{
    ...
    if (spec.profilePoints >= 0)
        parts.append(QString::fromUtf8("%1 点/轮廓").arg(spec.profilePoints));
...
{
    if (value < 0)
        return QString::fromUtf8("未公开");
    return value > 0 ? QString::fromUtf8("是") : QString::fromUtf8("否");
}
...
{
    if (rawSpecs.isEmpty())
        return QString::fromUtf8("<p>未公开</p>");
    ...
    for (const QString &key : keys) {
        ...
        if (value.isArray())
            ...
            text = value.toBool() ? QString::fromUtf8("是") : QString::fromUtf8("否");
        else
            text = value.toString();
    ...
    : QWidget(parent)
{
    ...
    layout->addWidget(pageTitle(QString::fromUtf8("3D 相机助手")));
    ...
    m_table->setHorizontalHeaderLabels({
        ...
        QString::fromUtf8("系列"),
        QString::fromUtf8("型号"),
        QString::fromUtf8("技术路线"),
        QString::fromUtf8("几何范围"),
        QString::fromUtf8("精度质量"),
...
{
    QGroupBox *box = new QGroupBox(QString::fromUtf8("需求过滤"));
    ...
    m_noMotionCheck = new QCheckBox(QString::fromUtf8("无需运动平台"));
    m_encoderCheck = new QCheckBox(QString::fromUtf8("需要编码器接口"));

    grid->addWidget(new QLabel(QString::fromUtf8("品牌")), 0, 0);
    grid->addWidget(m_brandCombo, 0, 1);
    grid->addWidget(new QLabel(QString::fromUtf8("技术路线")), 0, 2);
    grid->addWidget(m_technologyCombo, 0, 3);
    grid->addWidget(new QLabel(QString::fromUtf8("接口")), 0, 4);
    grid->addWidget(m_interfaceCombo, 0, 5);

    grid->addWidget(new QLabel(QString::fromUtf8("目标 X 覆盖")), 1, 0);
    grid->addWidget(m_xCoverageSpin, 1, 1);
    grid->addWidget(new QLabel(QString::fromUtf8("目标 Y 覆盖")), 1, 2);
    grid->addWidget(m_yCoverageSpin, 1, 3);
    grid->addWidget(new QLabel(QString::fromUtf8("Z 量程")), 1, 4);
    grid->addWidget(m_zRangeSpin, 1, 5);

    grid->addWidget(new QLabel(QString::fromUtf8("工作/参考距离")), 2, 0);
    grid->addWidget(m_workingDistanceSpin, 2, 1);
    grid->addWidget(new QLabel(QString::fromUtf8("最大 Z 重复精度")), 2, 2);
    grid->addWidget(m_zRepeatabilitySpin, 2, 3);
    grid->addWidget(new QLabel(QString::fromUtf8("最小速度")), 2, 4);
    grid->addWidget(m_speedSpin, 2, 5);

    grid->addWidget(new QLabel(QStringLiteral("IP")), 3, 0);
    grid->addWidget(m_ipCombo, 3, 1);
    grid->addWidget(new QLabel(QString::fromUtf8("材质场景")), 3, 2);
    grid->addWidget(m_materialCombo, 3, 3);
    grid->addWidget(m_noMotionCheck, 3, 4);
    grid->addWidget(m_encoderCheck, 3, 5);

    QPushButton *applyButton = new QPushButton(QString::fromUtf8("应用筛选"));
    QPushButton *clearButton = new QPushButton(QString::fromUtf8("清空条件"));
...
void ThreeDCameraPage::showDetailsForRow(int row)
{
    ...
    const QString missing = match.missingFields.isEmpty()
        ? QString::fromUtf8("无")
        : match.missingFields.join(QString::fromUtf8("；"));
    const QString reasons = match.rejectionReasons.isEmpty()
        ? QString::fromUtf8("无")
        : match.rejectionReasons.join(QString::fromUtf8("；"));
    ...
    html += line(QString::fromUtf8("匹配状态"), htmlEscape(threeDMatchStatusLabel(match.status)));
    html += line(QString::fromUtf8("技术路线"), htmlEscape(spec.technologyLabel));
    html += line(QString::fromUtf8("产品状态"), htmlEscape(spec.status));
    html += QStringLiteral("<b>%1</b>: <a href=\"%2\">%2</a><br>")
        .arg(htmlEscape(QString::fromUtf8("官方来源")), htmlEscape(spec.sourceUrl));
    html += line(QString::fromUtf8("采集日期"), htmlEscape(spec.sourceDate));
    html += QStringLiteral("</p>");
    html += QStringLiteral("<p>");
    html += line(QString::fromUtf8("缺失字段"), htmlEscape(missing));
    html += line(QString::fromUtf8("不适配原因"), htmlEscape(reasons));
    html += QStringLiteral("</p>");

    html += QStringLiteral("<h4>%1</h4><p>%2</p>")
        .arg(htmlEscape(QString::fromUtf8("几何覆盖")), htmlEscape(geometrySummary(spec)));
    html += QStringLiteral("<h4>%1</h4><p>%2<br>%3</p>")
        .arg(htmlEscape(QString::fromUtf8("精度质量")), htmlEscape(qualitySummary(spec)),
            line(QString::fromUtf8("测试条件"), htmlEscape(spec.accuracyCondition.isEmpty() ? QString::fromUtf8("未公开") : spec.accuracyCondition)));
    html += QStringLiteral("<h4>%1</h4><p>%2<br>%3%4</p>")
        .arg(htmlEscape(QString::fromUtf8("速度采样")), htmlEscape(speedSummary(spec)),
            line(QString::fromUtf8("需要外部运动"), htmlEscape(yesNoUnknown(spec.requiresExternalMotion))),
            line(QString::fromUtf8("支持编码器"), htmlEscape(yesNoUnknown(spec.supportsEncoder))));
    html += QStringLiteral("<h4>%1</h4><p>%2%3%4</p>")
        .arg(htmlEscape(QString::fromUtf8("光学与集成")),
            line(QString::fromUtf8("光源"), htmlEscape(spec.lightSource.isEmpty() ? QString::fromUtf8("未公开") : spec.lightSource)),
            line(QString::fromUtf8("波长"), htmlEscape(valueOrUnknown(spec.wavelengthNm, QStringLiteral(" nm"), 0))),
            line(QString::fromUtf8("接口"), htmlEscape(spec.interfaces.join(QStringLiteral(", ")))));
    html += QStringLiteral("<h4>%1</h4><p>%2%3%4%5%6</p>")
        .arg(htmlEscape(QString::fromUtf8("结构环境")),
            line(QStringLiteral("IP"), htmlEscape(spec.ipRating.isEmpty() ? QString::fromUtf8("未公开") : spec.ipRating)),
            line(QString::fromUtf8("结构"), htmlEscape(spec.structure.isEmpty() ? QString::fromUtf8("未公开") : spec.structure)),
            line(QString::fromUtf8("尺寸"), htmlEscape(spec.dimensions.isEmpty() ? QString::fromUtf8("未公开") : spec.dimensions)),
            line(QString::fromUtf8("重量"), htmlEscape(valueOrUnknown(spec.weightG, QStringLiteral(" g"), 0))),
            line(QString::fromUtf8("温度"), htmlEscape(spec.temperature.isEmpty() ? QString::fromUtf8("未公开") : spec.temperature)));
    if (!spec.materialScenarios.isEmpty())
        html += QStringLiteral("<p><b>%1</b>：%2</p>").arg(QString::fromUtf8("材质场景"), htmlEscape(spec.materialScenarios.join(QString::fromUtf8("，"))));
    if (!spec.notes.isEmpty())
        html += QStringLiteral("<p><b>%1</b>：%2</p>").arg(QString::fromUtf8("备注"), htmlEscape(spec.notes.join(QString::fromUtf8("；"))));
    html += QStringLiteral("<h4>%1</h4>%2").arg(QString::fromUtf8("官方原始规格字段"), rawSpecsHtml(spec.rawSpecs));
